...
namespace DataImport.Drivers
{
    public class CassandraDriver : IDisposable
    {
        ...
        /// <summary>
        /// Aggregates the existing records in the database to be used
        /// when directly uploading data.
        /// </summary>
        public List<AutocompleteRecord> AggregateExistingRecords()
        {
            ...
            {
                RowSet resultSet = session.Execute($"SELECT * FROM {autocompleteTable}");
                // Loop through the results
                foreach(Row row in resultSet.Where(x => x.GetValue<string>("name") == x.GetValue<string>("commonname")))
                {
                    records.Add(new AutocompleteRecord
                    {
                        CommonName = row.GetValue<string>("commonname"),
        ...
        {
            Program.LogInColor($"Cassandra: Adding {node.CommonName}", ConsoleColor.DarkMagenta);
            ...
            foreach(KeyValuePair<string, Tuple<string, string>> bindingEntry in bindingDictionary)
            {
                ...
                Program.LogInColor($"\tAdded {bindingEntry.Key} to main table", ConsoleColor.DarkMagenta);
            }
        }
